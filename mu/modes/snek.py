...
class SnekREPLConnection(REPLConnection):
    ...
    def set_ready(self):
        """
        Device is ready, send any pending output
        """
        self.ready = True
    ...
    def open(self):
        ...
        # something before talking to it. Don't wait more than two
        # seconds, in case it doesn't actually reset

        if self.wait_for_data:
            self.ready = False
            ready_func = lambda: self.set_ready()
            QTimer.singleShot(2000, ready_func)
    ...
    def _on_serial_read(self):
        ...
        # when we receive and ACK, restart transmission
        if b"\x06" in data:
            self.recv_ack()
...
class SnekMode(MicroPythonMode):
    ...
    valid_boards = [
        ...
        (0x0403, 0x6001, None, "Arduino Duemilanove with FT232"),
