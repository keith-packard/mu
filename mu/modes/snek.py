...
class SnekMode(MicroPythonMode):
    ...
    def add_repl(self):
        ...
        if device:
            baudrate = 38400
            if (device.vid, device.pid) not in self.slow_boards:
                baudrate = 115200
            try:
                if not self.connection:
                    ...
                    if (device.vid, device.pid) in self.reset_boards:
                        self.connection.serial.waitForReadyRead(10000)
                        time.sleep(0.2)

                    if self.force_interrupt:
                        # Send a Control-O / exit raw mode.
                        # Send a Control-C / keyboard interrupt.
                        self.connection.write(b"\x0f\x03")

                print("adding snek_repl view")
